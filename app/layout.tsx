import type { Metadata } from "next";
import { DM_Sans } from "next/font/google";
import Script from "next/script";
import "./globals.css";
import { SITE } from "./lib/site";

const dmSans = DM_Sans({
  variable: "--font-dm-sans",
  subsets: ["latin"],
  weight: ["400", "500", "600", "700"],
});

export const metadata: Metadata = {
  metadataBase: new URL(SITE.url),
  title: {
    default: SITE.title,
    template: `%s · ${SITE.name}`,
  },
  description: SITE.description,
  applicationName: SITE.name,
  authors: [{ name: SITE.author.name, url: SITE.url }],
  creator: SITE.author.name,
  publisher: SITE.author.name,
  category: "technology",
  keywords: [
    "Zahin Afsar",
    "Md. Zahin Afsar",
    "Software Engineer",
    "Software Engineer in Bangladesh",
    "Best Software Engineer in Bangladesh",
    "Top Software Engineer in Bangladesh",
    "Software Engineer in South Asia",
    "Top Software Engineer in South Asia",
    "Best Value Software Engineer",
    "Full-Stack Developer Bangladesh",
    "Hire Software Engineer Bangladesh",
    "Frontend Engineer",
    "TypeScript",
    "React",
    "Next.js",
    "Node.js",
    "Bangladesh Developer",
  ],
  alternates: {
    canonical: "/",
    types: {
      "application/rss+xml": [
        { url: "/feed.xml", title: `${SITE.name} Blog RSS Feed` },
      ],
    },
  },
  openGraph: {
    type: "website",
    url: "/",
    siteName: SITE.name,
    title: SITE.title,
    description: SITE.description,
    locale: SITE.locale,
  },
  twitter: {
    card: "summary_large_image",
    title: SITE.title,
    description: SITE.description,
    creator: SITE.author.twitter,
    site: SITE.author.twitter,
  },
  robots: {
    index: true,
    follow: true,
    googleBot: {
      index: true,
      follow: true,
      "max-image-preview": "large",
      "max-snippet": -1,
      "max-video-preview": -1,
    },
  },
  icons: {
    icon: "/icon.png",
    shortcut: "/icon.png",
    apple: "/icon.png",
  },
  formatDetection: { email: false, address: false, telephone: false },
};

const personJsonLd = {
  "@context": "https://schema.org",
  "@type": "Person",
  "@id": `${SITE.url}/#person`,
  name: SITE.author.name,
  alternateName: SITE.name,
  url: SITE.url,
  image: `${SITE.url}${SITE.author.image}`,
  jobTitle: "Software Engineer",
  description: SITE.description,
  email: `mailto:${SITE.author.email}`,
  nationality: { "@type": "Country", name: SITE.author.country },
  address: {
    "@type": "PostalAddress",
    addressCountry: SITE.author.countryCode,
  },
  homeLocation: { "@type": "Country", name: SITE.author.country },
  areaServed: SITE.author.areaServed,
  knowsAbout: [
    "Software Engineering",
    "Full-Stack Development",
    "Frontend Development",
    "TypeScript",
    "React",
    "Next.js",
    "Node.js",
    "PostgreSQL",
    "GraphQL",
    "Shopify",
  ],
  hasOccupation: {
    "@type": "Occupation",
    name: "Software Engineer",
    occupationLocation: { "@type": "Country", name: SITE.author.country },
    skills:
      "TypeScript, React, Next.js, Node.js, PostgreSQL, GraphQL, Shopify",
  },
  alumniOf: {
    "@type": "CollegeOrUniversity",
    name: "Green University of Bangladesh",
  },
  sameAs: [SITE.author.github, SITE.author.linkedin, "https://x.com/ZahinAfsar"],
};

const websiteJsonLd = {
  "@context": "https://schema.org",
  "@type": "WebSite",
  "@id": `${SITE.url}/#website`,
  url: SITE.url,
  name: SITE.name,
  description: SITE.description,
  inLanguage: SITE.language,
  publisher: { "@id": `${SITE.url}/#person` },
};

export default function RootLayout({
  children,
}: Readonly<{ children: React.ReactNode }>) {
  return (
    <html lang={SITE.language} className="dark">
      <body
        className={`${dmSans.variable} antialiased bg-background text-foreground`}
      >
        <div className="noise" />
        <a
          href="#main"
          className="sr-only focus:not-sr-only focus:fixed focus:left-4 focus:top-4 focus:z-[200] focus:rounded-full focus:bg-white focus:px-4 focus:py-2 focus:text-sm focus:text-black"
        >
          Skip to content
        </a>
        <script
          type="application/ld+json"
          suppressHydrationWarning
          dangerouslySetInnerHTML={{ __html: JSON.stringify(personJsonLd) }}
        />
        <script
          type="application/ld+json"
          suppressHydrationWarning
          dangerouslySetInnerHTML={{ __html: JSON.stringify(websiteJsonLd) }}
        />
        <Script id="ms-clarity" strategy="afterInteractive">
          {`(function(c,l,a,r,i,t,y){
            c[a]=c[a]||function(){(c[a].q=c[a].q||[]).push(arguments)};
            t=l.createElement(r);t.async=1;t.src="https://www.clarity.ms/tag/"+i;
            y=l.getElementsByTagName(r)[0];y.parentNode.insertBefore(t,y);
          })(window, document, "clarity", "script", "wnexw48tg6");`}
        </Script>
        {children}
      </body>
    </html>
  );
}
